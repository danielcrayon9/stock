...
- 공식 KOSPI 200 / KOSDAQ 150 전체 구성종목은 KRX 등 권위 있는 외부 소스 연동이 필요합니다. 외부 소스가 없으면 번들된 대표 종목 시드로 유니버스를 구성하며 이는 `데이터 부족` 상태로 간주합니다. `universe_constituents` 시트에 종목을 채우거나 `lib/universeService.ts`의 `refreshUniverse`를 외부 소스 연동으로 확장하세요.
